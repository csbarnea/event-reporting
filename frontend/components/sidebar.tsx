...
export function Sidebar() {
  const { filters, setFilter, clearFilters, loadEvents, isLoading } = useEventStore()
  ...
  return (
    <>
        ...
        <div className="flex h-full flex-col">
          ...
          <div className="flex-1 space-y-6 overflow-y-auto p-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="time-range">Interval raportare</Label>
              <Select
                value={timeRange}
                    ...
                    <Badge
                      ...
                      onClick={() => {
                        if (isActive) {
                          setFilter(
                            "tags",
                            currentTags.filter((t: string) => t !== type.value),
                          );
                        } else {
                          setFilter("tags", [...currentTags, type.value]);
                        }
                      }}
                    >
                      {type.label}
          ...
          <div className="border-t p-4">
            <Button className="w-full gap-2" onClick={() => loadEvents()} disabled={isLoading}>
      ...
      {/* Overlay for mobile */}
      {isOpen && <div className="fixed inset-0 z-10 bg-black/50 md:hidden" onClick={() => setIsOpen(false)} />}
    </>
  )
}
